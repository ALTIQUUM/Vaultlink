...
class RiskEngine:
    def analyze(self, tickers: list[str]) -> PortfolioRisk:
        ...
        prices = yf.download(
            symbols + ["SPY"], period="1y", progress=False, auto_adjust=True
        )["Close"].dropna()
        returns = prices.pct_change().dropna()
        metrics = [
            self._metric(symbol, returns[symbol], returns["SPY"]) for symbol in symbols
        ]
        ...
        cells = [
            CorrelationCell(
                ticker_a=a,
                ticker_b=b,
                correlation=Decimal(str(round(float(corr.loc[a, b]), 4))),
            )
            for a in symbols
            for b in symbols
        ]
                ...
                round(
                    (
                        float(
                            ...
                            - np.mean(
                                np.abs(
                                    corr.values[np.triu_indices_from(corr.values, 1)]
                                )
                            )
                        )
                        if len(symbols) > 1
        ...
        return PortfolioRisk(
            ...
            sector_diversification_score=score,
        )

    def _metric(
        self, ticker: str, returns: pd.Series, benchmark: pd.Series
    ) -> RiskMetric:
        ...
        return RiskMetric(
            ...
            volatility_90d=Decimal(
                str(round(float(r.tail(90).std() * np.sqrt(252)), 4))
            ),
